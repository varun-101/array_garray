...
This PR was automatically generated by the AI implementation system.`;
        ...
        console.log(`Creating pull request with GitHub CLI for branch: ${branchName}`);
        ...
        console.log('GitHub CLI stdout:', result.stdout);
        if (result.stderr) {
          console.log('GitHub CLI stderr:', result.stderr);
        }
        return { success: true, url: prUrl, branchName };
        
      } catch (ghError) {
        console.log('GitHub CLI error details:');
        console.log('Error message:', ghError.message);
        console.log('Error stdout:', ghError.stdout);
        console.log('Error stderr:', ghError.stderr);
        console.log('Full error output for debugging:', JSON.stringify({
          message: ghError.message,
          stdout: ghError.stdout,
          stderr: ghError.stderr,
          code: ghError.code
        }, null, 2));
        
        // Check if the error is actually a success case where PR was created but with warnings
        // Look for PR URL in both stdout and stderr
        const fullOutput = (ghError.stdout || '') + (ghError.stderr || '');
        const prUrlMatch = fullOutput.match(/https:\/\/github\.com\/[^\/]+\/[^\/]+\/pull\/\d+/);
        
        if (prUrlMatch) {
          const prUrl = prUrlMatch[0];
          console.log('Pull request created (with warnings):', prUrl);
          return { success: true, url: prUrl, branchName };
        }
        
        // Also check stdout specifically
        if (ghError.stdout && ghError.stdout.includes('https://github.com') && ghError.stdout.includes('/pull/')) {
          // Extract PR URL from stdout even if there was an error
          const prUrl = ghError.stdout.trim();
          console.log('Pull request created (with warnings):', prUrl);
          return { success: true, url: prUrl, branchName };
        }
        
        // Check if the PR already exists (common case)
        if (ghError.message && ghError.message.includes('already exists')) {
          console.log('Pull request already exists for this branch');
          // Try to get the existing PR URL
          try {
            const existingPR = await execAsync(`cd ${projectDir} && gh pr list --head ${branchName} --json url --jq '.[0].url'`);
            const prUrl = existingPR.stdout.trim();
            if (prUrl && prUrl !== 'null') {
              console.log('Found existing pull request:', prUrl);
              return { success: true, url: prUrl, branchName };
            }
          } catch (listError) {
            console.warn('Could not retrieve existing PR URL:', listError.message);
          }
        }
        
        // Final fallback: check if PR was actually created despite the error
        console.log('Attempting to verify if PR was created despite error...');
        try {
          const existingPR = await execAsync(`cd ${projectDir} && gh pr list --head ${branchName} --json url --jq '.[0].url'`);
          const prUrl = existingPR.stdout.trim();
          if (prUrl && prUrl !== 'null' && prUrl !== '') {
            console.log('PR was actually created despite error:', prUrl);
            return { success: true, url: prUrl, branchName };
          }
        } catch (verifyError) {
          console.warn('Could not verify PR creation:', verifyError.message);
        }
        
        console.warn('GitHub CLI PR creation failed:', ghError.message);
        return { 
          success: false, 
          error: 'GitHub CLI not available or PR creation failed', 
          branchName,
      ...
      // Check if this might be a case where the PR was actually created
      // but there was an error in the process (e.g., network issues, timeouts)
      if (error.message && (
        ...
        // Try to verify if PR was actually created despite the error
        try {
          const existingPR = await execAsync(`cd ${projectDir} && gh pr list --head ${branchName} --json url --jq '.[0].url'`);
          const prUrl = existingPR.stdout.trim();
          if (prUrl && prUrl !== 'null') {
